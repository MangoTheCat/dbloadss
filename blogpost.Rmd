...
```{r, runmodelodbc}
library(dbloadss)
output_data <- simulate_departure_delays(flights, nsim = 20)
dim(output_data)
```

So for 20 simulations we have just short of 4 million rows of output! It's just a flight ID (for joining back to the source), a simulation ID, and a delay.
...
DROP PROC IF EXISTS simulate_flight_delays;
GO
CREATE PROC simulate_flight_delays(@nsim int = 20)
AS
BEGIN
 EXEC sp_execute_external_script
   ...
   , @script = N'  
          library(dbloadss)
          output_data <- simulate_flight_delays(input_data, nsim = nsim_r)
' 
...
EXEC [dbo].[simulate_flight_delays] @nsim = 20
